            ...
            <div class="modal-body p-0">
                ...
                @if ($productModalCount)
                    ...
                    <div class="col-lg-6 p-lg-0">
                        {{-- <a class="d-none" href="{{ asset('assets/products/'.$productModal->firstMedia->file_name) }}" title="{{ $productModal->name }}" data-lightbox="productview"></a>
                         --}}
                        <a class="product-view d-block h-100 bg-cover bg-center"
                        style="background: url('{{ asset('assets/products/'.$productModal->firstMedia->file_name) }}')"
                        href="{{ asset('assets/products/'.$productModal->firstMedia->file_name) }}" data-lightbox="productview"
                        ...
                        {{-- @foreach ($productModal->media as $media)
                            ...
                            @else
                                <a class="d-none" href="{{ asset('assets/products/'.$media->file_name) }}" title="{{ $productModal->name }}" data-lightbox="productview"></a>
                            @endif
                        @endforeach --}}
                    </div>

                    <div class="col-lg-6">
                        <button class="btn-close p-4 position-absolute top-0 end-0 z-index-20 shadow-0" type="button" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button>
